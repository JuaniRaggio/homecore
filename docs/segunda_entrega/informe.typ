...
== Gestión de cuenta y autenticación (RF1-6)
...
== Gestión y control de dispositivos (RF7-9)

- *Gestión de dispositivos (RF7):* El usuario puede crear dispositivos asignándoles un nombre y tipo, renombrarlos, editarlos y eliminarlos. Toda eliminación requiere confirmación explícita para evitar acciones accidentales.

- *Consulta de dispositivos (RF8):* El sistema presenta un listado de todos los dispositivos del hogar mostrando nombre, tipo, estado actual y habitación asignada. Además, existe una vista de detalle por dispositivo que expone su estado completo y sus controles específicos, actualizada en tiempo real.
...
== Gestión completa de rutinas (RF10-12, 23)

El sistema permite gestionar rutinas automatizadas que ejecutan múltiples acciones sobre dispositivos del hogar.

- *Creación y edición de rutinas (RF10):* El usuario puede crear rutinas mediante un asistente de 4 pasos donde define:
...
== Consultar acciones realizadas (RF13)
...
== Gestión de habitaciones y vinculación de dispositivos (RF14-16)

- *Gestión de habitaciones (RF14-15):* El usuario puede crear habitaciones dentro de un hogar, renombrarlas y eliminarlas. Toda eliminación requiere confirmación explícita. El sistema lista las habitaciones del hogar seleccionado, y al ingresar al detalle de una habitación, se visualizan los dispositivos que contiene con acceso directo al control de cada uno.
...
== Gestión y consulta de hogares (RF17-19)

El sistema permite gestionar múltiples hogares de forma completa:

- *Gestión de hogares (RF17):* El usuario puede crear hogares con un nombre identificatorio, editarlos y eliminarlos. Adicionalmente, se implementó la posibilidad de compartir un hogar con otros usuarios mediante su correo electrónico, otorgándoles acceso a sus dispositivos y habitaciones.

- *Consulta de hogares (RF18):* El sistema presenta un panel general con todos los hogares a los que tiene acceso el usuario, tanto propios como compartidos, junto con un resumen del estado de sus dispositivos. Desde este panel se navega al detalle de cada hogar.

- *Vínculo de habitaciones a hogares (RF19):* Cada habitación pertenece exclusivamente a un hogar. El usuario puede crear habitaciones dentro del contexto de un hogar específico, y cada dispositivo debe estar vinculado a una habitación. No se permiten dispositivos sin asignar a una habitación ni habitaciones sin asignar a un hogar. Esta estructura jerárquica garantiza una organización clara y coherente de los elementos del sistema.
...
== Consultar consumo eléctrico (RF22)
...
== Wave (RNF9)
La validación de accesibilidad correspondiente al requerimiento RNF se realizó utilizando herramientas basadas en axe-core y criterios WCAG 2.x (Web Content Accessibility Guidelines) nivel AA (medio). Inicialmente se detectaron problemas relacionados con la ausencia del elemento semántico `<main>`, contenido fuera de regiones accesibles y contrastes insuficientes en algunos componentes de interfaz.
...
Los mensajes de error proporcionan información clara sobre qué salió mal y cómo proceder, cumpliendo con la *ayudar a los usuarios a reconocer, diagnosticar y recuperarse de errores* _(Heurística de Nielsen #9)_.
...
Los diálogos de confirmación implementan la *prevención de errores* _(Heurística de Nielsen #5)_, solicitando confirmación antes de acciones destructivas.
...
*Justificación:* Esta medida busca *prevenir errores accidentales* _(Heurística de Nielsen #5)_. En el caso de las habitaciones, se informa adicionalmente sobre el impacto en los dispositivos vinculados para que el usuario tome una decisión informada.
...
*Justificación:* Esta vista centraliza la información más consultada, permitiendo al usuario realizar acciones frecuentes sin necesidad de navegar por las distintas secciones. Esto favorece el *reconocimiento sobre el recuerdo* _(Heurística de Nielse #6)_ al presentar el estado general de forma inmediata.
...
*Justificación:* Informar claramente al usuario sobre su ubicación dentro de la jerarquía de la aplicación facilita la navegación y proporciona libertad de movimiento con salidas de emergencia claras, lo cual reenfuerza *el control y la libertad del usuario* _(Heurística de Nielsen #3)_.
...
*Decisión:* Se priorizó el uso de tarjetas de resumen y métricas clave sobre la representación gráfica del plano de la casa. Por ende se optó por eliminar la vista isométrica 3D del hogar y reemplazarla por una presentación más directa de la información.

*Justificación:* El diseño original contemplaba un plano isométrico con la ubicación espacial de los dispositivos. Dado que la API no proporciona datos de posicionamiento, se optó por un enfoque basado en tarjetas de información. Esto asegura que el usuario reciba datos precisos y accionables sobre sus dispositivos favoritos y el consumo energético, manteniendo la claridad informativa para usuarios como *Carolina*, que buscan monitorear su hogar rápidamente. Esta decisión también fue tomada en respuesta al feedback de la primera entrega, donde se identificó que la vista isométrica no aportaba valor funcional y complicaba innecesariamente la implementación.
